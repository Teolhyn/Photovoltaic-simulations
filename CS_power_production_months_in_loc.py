...
if __name__ == '__main__':
    ...
    times = pd.date_range('2021-01-01', '2021-12-31', freq='1min', tz=tz)
    location = Location(40.42, -3.70, 'Europe/Madrid', 650, name='Madrid')
...
# ----------------------------------------------------------------------------
    ...
    # Plotting
    for month, colour in zip(['March', 'June', 'September', 'December'], ['r', 'g', 'b', 'k']):
        plt.plot(bifacial_avgs[month], colour, label=str(month))
        plt.plot(monofacial_avgs[month], colour)

    plt.xticks(["00:00", "03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00"])
    plt.xlabel('Time')
    plt.ylabel('AC Power (W)')
    plt.title('Average power profile of a month in clear sky conditions')
    plt.legend()
    plt.show()

    print(bifacial_avgs)
    print(monofacial_avgs)
